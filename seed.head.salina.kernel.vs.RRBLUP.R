...
these.regions.of.interest <- read.csv("Chr_bp_Pos_Input_File_main_seed_head_Salina.csv", head = TRUE)
...
dir.create(paste("Results_Yeild",colnames(myY.for.GS)[2],"MultiBLUP",sep = ""))
multi.BLUP.kfoldfoldCV(Y = myY.for.GS, Geno = myG, traitname = colnames(myY.for.GS)[2], 
                       path.for.results = paste("Results_Yeild",colnames(myY.for.GS)[2],"MultiBLUP","/",sep = ""), 
