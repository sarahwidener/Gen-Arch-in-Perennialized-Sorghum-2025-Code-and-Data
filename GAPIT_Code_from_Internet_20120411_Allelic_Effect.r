...
if(is.null(Y) & is.null(GP)) return (list(GWAS=NULL,GPS=NULL,compression=NULL,kinship.optimum=NULL,kinship=myGenotype$KI,PCA=myGenotype$PC,GD=cbind(GT,GD),G=myGenotype$G))

#In case of null Y, return genotype only          
if(is.null(Y)) return (list(GWAS=NULL,GPS=NULL,compression=NULL,kinship.optimum=NULL,kinship=myGenotype$KI,PCA=myGenotype$PC,GD=cbind(GT,GD),G=myGenotype$G))
...
Memory=GAPIT.Memory(Memory=Memory,Infor="Manhattan plot")
  ...
  GWAS=PWIP$PWIP[PWIP$PWIP[,10]<=SNP.FDR,]
...
#  temp <- mt.reject(adjp[,2], FDR.Rate)
  ...
  colnames(PWIP)[10] <- "FDR_Adjusted_P-values"
...
################################################################################
GAPIT.SUPER.FastMLM <- function(ys, xs, vg, delta, Z = NULL, X0 = NULL, snp.pool=NULL,LD=0.01,method="FaST") {
#Input: ys, xs, vg, delta, Z, X0, snp.pool
#Output: GWAS
#Authors: Qishan Wang, Feng Tian and Zhiwu Zhang
#Last update: November 11, 2011
...
LDsqr=sqrt(LD)
K.X= snp.pool
K<-as.matrix(K.X%*%t(K.X))#####2012.2.8
if(method!="SUPER")
{
 ...
 U <- eig.L$vectors*matrix(sqrt(1/(eig.L$values + delta)), t, t, byrow = TRUE) 
